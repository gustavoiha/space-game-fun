...
4. Bug4 [DONE]
  - Star and wormhole icons in the star system map now scale based on real in-system sizes (star radius and gate collider bounds) so they no longer dwarf map distances.
...
- **M2.11 – Star System Map Render Overhaul [TODO]**
  - Revamp the star system map to use a dedicated top-down orthographic camera rendering the actual in-system scene (anchored to the current `StarSystemRuntime` center) instead of purely icon-based UI.
  - Restrict the camera’s culling mask and render layers to key objects (star, gates, ships, nav markers) with simplified materials/LOD so map view stays performant and uncluttered; hide dense VFX/props and cap draw distance.
  - Map UI controls (open/close, zoom level) should drive this camera’s position/size, preserving the existing default “system-first” view while preventing regressions to galaxy view switching; document any new layers, prefabs, or required scene components in `UNITY_CONFIGURATION.md`.
...
- **M3.1 – System View Scene/Layout [TODO]**
  - Finalise the layout for a single star system **scene**:
    ...
    - Display stations/colonies (when dONE).
